...
const GAMEPAD_PRESETS = {
  ...
  responsive: {
    gamepadJoystickHz: 60,
    gamepadButtonHz: 20,
    gamepadUiHz: 60,
    gamepadAxisEpsilon: 0.005,
    gamepadUseVolatile: true,
    gamepadDisableCompression: true
  },
  lowBandwidth: {
    gamepadJoystickHz: 15,
    gamepadButtonHz: 5,
    gamepadUiHz: 30,
    gamepadAxisEpsilon: 0.02,
    gamepadUseVolatile: true,
    gamepadDisableCompression: false
  }
};
...
const Settings = ({ isOpen, onClose, onSave, initialSettings }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const merged = applyLegacyConnection(mergeSettings(defaultSettings, initialSettings));
  ...
  const handleGamepadPresetChange = useCallback((preset) => {
    setSettings(prev => {
      const control = {
        ...prev.control,
        gamepadPreset: preset
      };
      const presetValues = GAMEPAD_PRESETS[preset];
      if (presetValues) {
        Object.assign(control, presetValues);
      }
      return {
        ...prev,
        control
  ...
  const handleGamepadSettingChange = useCallback((key, value) => {
    setSettings(prev => {
      const control = {
        ...prev.control,
        [key]: value
      };
      if (control.gamepadPreset !== 'custom') {
        control.gamepadPreset = 'custom';
      }
      return {
        ...prev,
        control
  ...
  const handleGamepadSliderChange = useCallback((key, value, min, max) => {
    const numValue = parseFloat(value);
    if (!isNaN(numValue) && numValue >= min && numValue <= max) {
      handleGamepadSettingChange(key, numValue);
    } else {
      console.warn(`[Settings handleGamepadSliderChange] Invalid slider value: ${value}`);
    }
  }, [handleGamepadSettingChange]);
      ...
      <div className="settings-modal" onClick={e => e.stopPropagation()}> 
        ...
        >
          <div className="settings-grid">
            ...
            <div className="settings-section">
              ...
              {expandedSections.control && (
                <div className="section-content">
                  ...
                  <div className="setting-item">
                    <label htmlFor="control-gamepad-preset">Gamepad Performance Preset</label>
                    <select
                      id="control-gamepad-preset"
                      value={getSetting('control', 'gamepadPreset', 'balanced')}
                      onChange={(e) => handleGamepadPresetChange(e.target.value)}
                    >
                      <option value="balanced">Balanced (30/10 Hz)</option>
                      <option value="responsive">Responsive (60/20 Hz)</option>
                      <option value="lowBandwidth">Low bandwidth (15/5 Hz)</option>
                      <option value="custom">Custom</option>
                    </select>
                  </div>
                  <div className="setting-item">
                    <label htmlFor="control-gamepad-joystick-hz">Joystick Emit Rate (Hz)</label>
                    <input
                      id="control-gamepad-joystick-hz"
                      type="range"
                      min="5"
                      max="120"
                      step="5"
                      value={getSetting('control', 'gamepadJoystickHz', 30)}
                      onChange={(e) => handleGamepadSliderChange('gamepadJoystickHz', e.target.value, 5, 120)}
                    />
                    <span className="setting-value">{getSetting('control', 'gamepadJoystickHz', 30)} Hz</span>
                  </div>
                  <div className="setting-item">
                    <label htmlFor="control-gamepad-button-hz">Button Emit Rate (Hz)</label>
                    <input
                      id="control-gamepad-button-hz"
                      type="range"
                      min="1"
                      max="60"
                      step="1"
                      value={getSetting('control', 'gamepadButtonHz', 10)}
                      onChange={(e) => handleGamepadSliderChange('gamepadButtonHz', e.target.value, 1, 60)}
                    />
                    <span className="setting-value">{getSetting('control', 'gamepadButtonHz', 10)} Hz</span>
                  </div>
                  <div className="setting-item">
                    <label htmlFor="control-gamepad-ui-hz">UI Update Rate (Hz)</label>
                    <input
                      id="control-gamepad-ui-hz"
                      type="range"
                      min="10"
                      max="120"
                      step="5"
                      value={getSetting('control', 'gamepadUiHz', 60)}
                      onChange={(e) => handleGamepadSliderChange('gamepadUiHz', e.target.value, 10, 120)}
                    />
                    <span className="setting-value">{getSetting('control', 'gamepadUiHz', 60)} Hz</span>
                  </div>
                  <div className="setting-item">
                    <label htmlFor="control-gamepad-axis-epsilon">Axis Change Threshold</label>
                    <input
                      id="control-gamepad-axis-epsilon"
                      type="range"
                      min="0.0"
                      max="0.1"
                      step="0.005"
                      value={getSetting('control', 'gamepadAxisEpsilon', 0.01)}
                      onChange={(e) => handleGamepadSliderChange('gamepadAxisEpsilon', e.target.value, 0.0, 0.1)}
                    />
                    <span className="setting-value">{getSetting('control', 'gamepadAxisEpsilon', 0.01).toFixed(3)}</span>
                  </div>
                  <div className="setting-item">
                    <label htmlFor="control-gamepad-volatile">Use Volatile Joystick Emits</label>
                    <input
                      id="control-gamepad-volatile"
                      type="checkbox"
                      checked={getSetting('control', 'gamepadUseVolatile', true)}
                      onChange={(e) => handleGamepadSettingChange('gamepadUseVolatile', e.target.checked)}
                    />
                  </div>
                  <div className="setting-item">
                    <label htmlFor="control-gamepad-compress">Disable Joystick Compression</label>
                    <input
                      id="control-gamepad-compress"
                      type="checkbox"
                      checked={getSetting('control', 'gamepadDisableCompression', true)}
                      onChange={(e) => handleGamepadSettingChange('gamepadDisableCompression', e.target.checked)}
                    />
                  </div>
                </div>
              )}
            ...
            <div className="settings-section">
              ...
              {expandedSections.visualization && (
                <div className="section-content">
                  ...
                  <div className="setting-item">
                    <label htmlFor="viz-tf-throttle">TF Throttle Rate (Hz)</label>
                    <input
                      id="viz-tf-throttle"
                      type="number"
                      min="1"
                      max="60"
                      step="1"
                      value={getSetting('visualization', 'tfThrottleRate', 10)}
                      onChange={(e) => handleChange('visualization', 'tfThrottleRate', parseInt(e.target.value, 10) || 10)}
                    />
                  </div>
                  <div className="setting-item">
            ...
            <div className="settings-section">
              ...
              {expandedSections.topics && (
                <div className="section-content">
                  ...
                  <div className="setting-item">
                    <label htmlFor="topics-telemetry-all">Legacy Telemetry</label>
                    <input
                      id="topics-telemetry-all"
                      type="text"
                      value={getSetting('topics', 'telemetry_all', '/robot/telemetry/all')}
                      onChange={(e) => handleChange('topics', 'telemetry_all', e.target.value)}
                    />
                  </div>
                </div>
              )}
                ...
                <div className="section-content">
                  <div className="setting-item">
                    ...
                    <input
                      id="conn-ros"
                      type="text"
                      value={getSetting('connection', 'rosBridgeUrl', `ws://${window.location.hostname}:9090`)}
                  ...
                  <div className="setting-item">
                    ...
                    <input
                      id="conn-socket"
                      type="text"
                      value={getSetting('connection', 'socketUrl', window.location.origin)}
                  ...
                  <div className="setting-item">
                    ...
                    <input
                      id="conn-video"
                      type="text"
                      value={getSetting('connection', 'webVideoHost', window.location.hostname)}
            ...
            {/* Genesis Section */}
            <div className="settings-section">
              <h3 onClick={() => toggleSection('genesis')} className="section-header">
                {renderChevron('genesis')} Genesis Simulation
              </h3>
              {expandedSections.genesis && (
                <div className="section-content">
                  <div className="setting-item">
                    <label htmlFor="genesis-stream-backend">Stream Backend</label>
                    <select
                      id="genesis-stream-backend"
                      value={getSetting('genesis', 'streamBackend', 'websocket')}
                      onChange={(e) => handleChange('genesis', 'streamBackend', e.target.value)}
                    >
                  ...
                  {getSetting('genesis', 'streamBackend', 'websocket') === 'webrtc' && (
                    <>
                      <div className="setting-item">
                        ...
                        <textarea
                          id="genesis-webrtc-stun"
                          rows={2}
                          value={(getSetting('genesis', 'webrtcStunUrls', ['stun:stun.l.google.com:19302']) || []).join('\n')}
                          onChange={(e) => handleChange('genesis', 'webrtcStunUrls', e.target.value.split('\n').map(s => s.trim()).filter(Boolean))}
                  ...
                  <div className="setting-item">
                    <label htmlFor="genesis-camera-res">Camera Resolution</label>
                    <select
                      id="genesis-camera-res"
                      value={getSetting('genesis', 'cameraRes', '1280x720')}
                      onChange={(e) => handleChange('genesis', 'cameraRes', e.target.value)}
                    >
                  ...
                  <div className="setting-item">
                    <label htmlFor="genesis-metrics-rate">Metrics Rate (Hz)</label>
                    <input
                      id="genesis-metrics-rate"
                      type="range"
                      min="1"
                      max="30"
                      step="1"
                      value={getSetting('genesis', 'trainingMetricsRate', 5)}
                      onChange={(e) => handleSliderChange('genesis', 'trainingMetricsRate', e.target.value, 1, 30)}
                    />
                    <span className="setting-value">{getSetting('genesis', 'trainingMetricsRate', 5)} Hz</span>
                  </div>
                  <div className="setting-item">
                    <label htmlFor="genesis-show-overlay">Show Sim Overlay</label>
                    <input
                      id="genesis-show-overlay"
                      type="checkbox"
                      checked={getSetting('genesis', 'showSimOverlay', true)}
                      onChange={(e) => handleChange('genesis', 'showSimOverlay', e.target.checked)}
                    />
                  </div>
                  <div className="setting-item">
                    <label htmlFor="genesis-show-pip">Show PiP Window</label>
                    <input
                      id="genesis-show-pip"
                      type="checkbox"
                      checked={getSetting('genesis', 'showPip', true)}
                      onChange={(e) => handleChange('genesis', 'showPip', e.target.checked)}
                    />
                  </div>
                  <div className="setting-item">
                    <label htmlFor="genesis-auto-connect">Auto-connect Bridge</label>
                    <input
                      id="genesis-auto-connect"
                      type="checkbox"
                      checked={getSetting('genesis', 'autoConnectBridge', true)}
                      onChange={(e) => handleChange('genesis', 'autoConnectBridge', e.target.checked)}
                    />
                  </div>
                </div>
              )}
